#!/usr/bin/env python3
"""run.py — executable core for meta-skill `meta-harness-refuter`.

Doctrine package: rig-adversarial-verification | Diamond: D2
Contract: plant-failure verify every gate in a harness
"""
    ...
    try:
        subprocess.run(
            ["rig-memory-os", "call", "memory.record_event", "--input", json.dumps({
                ...
                "run_id": "meta-harness-refuter-run-" + datetime.now(timezone.utc).strftime("%Y%m%d"),
                "actor": "meta-harness-refuter",
                "operator_scope": "operator",
                "purpose": "meta-skill execution",
                "sensitivity": "internal",
                "event_type": event_type,
                "occurred_at": datetime.now(timezone.utc).isoformat(),
                "idempotency_key": "meta-harness-refuter-" + datetime.now(timezone.utc).strftime("%Y%m%d%H%M%S"),
                "sequence": 1,
                "payload": payload,
            })],
...
def core(payload: dict) -> dict:
    """Core mechanical check for meta-harness-refuter."""
...
def main() -> int:
    ...
    try:
        ...
        return 1
    result = core(payload)
    record_event("skill.meta-harness-refuter.executed", {"result_status": result["status"], "target": payload.get("target", "")})
